"""Module to scrape the categories from the biorxiv collection."""
...
def scrape_categories() -> list[str]:
    """Scrape the categories from the biorxiv collection.
    ...
    url = "https://www.biorxiv.org/collection"
...
def get_biorxiv_categories() -> pd.DataFrame:
    """Get the categories from the biorxiv collection.
    ...
    response = model.invoke(
        "You are given a list of categories from the biorxiv collection. For each of the following categories, please provide a short description of the category (1-2 sentences maximum)"
    )
    ...
    response_df["database"] = "biorxiv"
    return response_df


if __name__ == "__main__":
    categories = get_biorxiv_categories()
    print(categories)
